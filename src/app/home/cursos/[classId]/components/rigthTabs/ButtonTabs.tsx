...
export const ButtonTabs = ({ activeTab, setActiveTab }: iProps) => {
    ...
    return (
        <nav className="rounded-lg border border-zinc-200 bg-white p-2 shadow-sm">
            <div className="flex flex-col gap-1 text-zinc-900">
                {tabs.map((tab) => {
                    ...
                    return (
                        <button
                            key={tab.id}
                            className={`flex w-full items-center gap-3 rounded-md px-3 py-2 text-left text-sm font-medium transition-colors ${selected
